...
CREATE TABLE `__new_issues` (
	...
	`date` text NOT NULL,
	`updated_at` text DEFAULT '2025-07-10T13:42:30.847Z',
	`created_at` text DEFAULT '2025-07-10T13:42:30.850Z',
	FOREIGN KEY (`motorcycle_id`) REFERENCES `motorcycle`(`id`) ON UPDATE no action ON DELETE no action
);
--> statement-breakpoint
INSERT INTO `__new_issues`("id", "motorcycle_id", "odo", "description", "priority", "date", "updated_at", "created_at") SELECT "id", "motorcycle_id", "odo", "description", "priority", "date", "updated_at", "created_at" FROM `issues`;--> statement-breakpoint
